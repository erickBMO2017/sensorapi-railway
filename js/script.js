...
function toggleTableVisibility() {
    ...
    const tableCard = document.getElementById("dataTable").closest(".card-body");
    const tableTitle = tableCard.querySelector(".card-title");
    const tableHeader = document.getElementById("tableHeader");

    if (operationType === "staff_schedule") {
        tableTitle.textContent = "Datos del Usuario";
    ...
    } else if (operationType === "sales" || operationType === "transactions") {
        tableTitle.textContent = "Datos de los Sensores";
...
function handleOperationTypeChange() {
    ...
    // Mostrar u ocultar fileSection para sales y staff_schedule
    const showFileSection = operationType === "sales" || operationType === "staff_schedule";
    fileSection.classList.toggle("d-none", !showFileSection);
    ...
    try {
        ...
        if (operationType === "staff_schedule") {
            if (headerRow.length !== 5) {
                document.getElementById("dataError").textContent =
                    "Error: El archivo para horario de personal debe tener exactamente 5 columnas.";
                return;
            }


            // Aquí podrías llamar a una función específica para procesar ese tipo de datos
            processStaffSchedule(sheet);
            return;
        }
...
async function handleFetchData() {
    ...
    if (operationType === "staff_schedule") {
        ...
        url = "/import_staff_schedule";
    } else {
        url = isDaily ? "/sales" : "/sales_hourly";
    }
...
function processStaffSchedule(sheet) {
    document.getElementById("tableBody").innerHTML = "";
    requests = [];
    let hasEmptyFields = false;
    let hasInvalidDates = false;
    ...
    for (let i = 1; i < sheet.length; i++) {
        ...
        // Validar que no haya campos vacíos en las 5 columnas
        const emptyFields = row.slice(0, 5).some(cell => cell === undefined || cell === "");
        if (emptyFields) {
            hasEmptyFields = true;
            continue;
        }
        ...
        requests.push(requestData);
        addStaffScheduleRow(requestData); // Esta función debe mostrar los datos en la tabla
    }
...
function addStaffScheduleRow(data) {
    ...
    nameCell.textContent = data.staffname;

    const codeCell = document.createElement("td");
    codeCell.textContent = data.staffcode;

    const startHourCell = document.createElement("td");
    startHourCell.textContent = data.starthour;

    const endHourCell = document.createElement("td");
    endHourCell.textContent = data.endhour;
